...
namespace aptm2019.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.lstRepartos = new SelectList(new BLL_MReparto().SelectAll(), "MRPT_nId", "cDescripcion");
            return View();
        }
        [HttpPost]
        public ActionResult GetDetallesReparto(int idReparto)
        {
            ENT_MReparto mrep = new BLL_MReparto().Select(idReparto);
            ENT_DServicioReparto dserv = new BLL_DServicioReparto().Select(mrep.MRPT_nIdServicioReparto);
            ENT_MServicioReparto mserv = new BLL_MServicioReparto().Select(dserv.DSVRP_nIdMServicioReparto);
            ENT_MVehiculo veh = new BLL_MVehiculo().Select(mserv.MSVRP_nIdMVehiculo);
            ENT_MEmpleado emp = new BLL_MEmpleado().Select(dserv.DSVRP_nIdMEmpleado);
            ...
            //string url = @"http://router.project-osrm.org/route/v1/driving/-77.017403,-12.029341;-77.125822,-12.056705";
            ...
            //dynamic va = JsonConvert.DeserializeObject<dynamic>(html);


            return Json(new
            {
                cEMP = emp.MEMP_cNombre + " " + emp.MEMP_cApePaterno + " " + emp.MEMP_cApeMaterno,
                cPLaca = veh.MVEH_cPlaca,
                cHorario = mrep.MRPT_dHInicio.ToString("dd/MM/yyyy HH:mm") + " - " + mrep.MRPT_dHFin.ToString("dd/MM/yyyy HH:mm"),
                oCarga
            }) ;
        }
        ...
        public ActionResult TestPlanning()
        {
            return View();
        }
        [HttpPost]
